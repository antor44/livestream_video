...
`Playlist4whisper` is a Graphical User Interface (GUI) that manages a playlist whose functions are executed by the `livestream_video.sh` bash script. Together, they can play local audio/video files, live soundcard inputs, or online video streams, transcribing the audio in real-time using advanced AI technology.

The application supports a fully configurable `Timeshift` feature, multi-instance and multi-user execution, and allows for both global and per-channel option management. It also offers high-quality online translation via **Google Gemini AI** or Google Translate, as well as Text-to-Speech (TTS) functionality through `translate-shell`. All of these tasks are designed to run efficiently, even on low-power processors.

Additionally, it generates standard subtitle files (`.srt`) from audio and video files. Users can leverage enhanced **cut and merge** functions to edit their media files, generate subtitles for selected segments, and even create separate subtitle files for different languages.


## Specialized Functions for Journalists, Content Creators, and Researchers

While this application is widely used to view, transcribe, and translate online video streams, it is especially valuable for professionals like journalists, content creators, and researchers who need a robust solution for categorizing large volumes of video and audio files with integrated subtitle transcriptions and translations.

-   **High-Quality Subtitles:** Subtitles for local video/audio files are generated by a local Whisper AI. This includes English translations if you use Whisper's internal translation option. Alternatively, you can translate subtitles online into numerous languages using the high-quality **Gemini AI** or Google Translate engines, with optional Text-to-Speech support via `translate-shell`.

-   **Flexible Performance:** On older computers, you can generate highly accurate subtitles using the largest AI models without the real-time processing constraints of live streams; the only limit is the processing time. Conversely, on a powerful CPU or with a GPU/NPU and a hardware-accelerated version of `whisper.cpp`, subtitles can be generated very quickly.

-   **Advanced Editing:** The option to **cut** video/audio files and subsequently **merge** their subtitles is a key feature. It supports merging only specific portions of subtitles, which is useful for creating clean, segmented transcripts. This tool can also help mitigate the occasional "hallucination" issues present in some Whisper AI models by allowing you to re-transcribe problematic sections.

-   **Broad File Support:** `playlist4whisper` supports all video and audio formats compatible with your system's installed version of `ffmpeg`. You can play audio files with subtitles using players like `VLC` or `mpv`. To display subtitles with audio files:
    *   **VLC:** Configure it to show a visualization (like a spectrometer) during audio playback.
    *   **MPV:** Add the `--force-window` option in `playlist4whisper`'s "Player Options" field and use the "Player Only" mode.
      

-   **Flexible Organization of Your Media Collection:** `playlist4whisper` accepts optional command-line arguments to add any number of tabs, specifying their names and colors displayed within the application. The names provided by the user will be used to create the M3U playlist and JSON configuration files, which will be converted to lowercase:
...
#### How to Enable Gemini Translation in `playlist4whisper` GUI**
...
-   **Multi-Platform:** Compatible with Linux and macOS.
-   **Audio Inputs:** Transcribe from any audio input, including loopback devices to capture desktop audio. Supported on Linux (PulseAudio), macOS (AVFoundation), and Windows (via WSL2).
...
##### 3. Select a Gemini Model in the GUI
