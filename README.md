...
`playlist4whisper` has been successfully tested on macOS versions for Mx or ARM without issues, on macOS Ventura (Intel), and also on an older version of playlist4whisper/livestream.sg (successful version 2.60 + whisper.cpp v. 1.42) on very old versions like Big Sur (although it requires some extra adjustments).
