...
A: This occurs because the application lacks a voice activity detection (VAD) system for splitting the audio during a silence.  The current versions of the script segments sound files into chunks based on user-selected durations, this may lead to the truncation or deletion of words at the beginning and end of each chunk. The IA for transcription, Whisper, incorporates a GPT2 model, a version prior to OpenAI's ChatGPT, that attempts to fill in these gaps and address other sound issues. This system works better when a larger Whisper model is chosen. Nonetheless, a VAD system is planned to be incorporated in the near future.
